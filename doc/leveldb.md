...
### 跳跃表（Skiplist）
 * 跳跃表最大的优点在于实现简单。单纯比较单线程性能，跳跃表和RB-Tree可以说相差不大，都是O(logN)，但跳跃表的实现比RB-Tree、AVL简单很多。
 * 在高并发场景下，如果需要更新数据，跳跃表没有繁琐的平衡过程，更新的部分相对较少，多线程竞争锁的代价就会减少，因此在高并发的环境下跳跃表的性能比RB-Tree要好。
