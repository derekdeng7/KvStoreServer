...
## Linux异步机制的信号(signal)处理流程
  * A进程调用信号发送函数，发送信号给B，信号并不是直接从进程A发送给进程B，而是要通过内核来进行转发，这样做的目的应该也是为了对进程的管理和安全因素考虑。所以A进程会进入内核态运行操作系统的信号调度代码；
  * 内核接受到这个信号消息后，会先检查A进程是否有权限对B进程的信号表对应的项进行设置，如果可以，就会对B进程的信号表进行设置，这里面信号处理有个特点，就是没有排队的机制，也就是说某个信号被设置之后，如果B进程还没有来及进行响应，那么如果后续第二个同样的信号消息过来，就会被阻塞掉，也就是丢弃。
  * 内核对B进程信号设置完成后，就会发送软中断请求给B进程。操作系统返回给A，A继续执行；
  * B进程分配的处理器时间用完了，被时钟硬件中断。时钟硬件中断处理函数准备挂起B进程，也就是把寄存器和函数堆栈保存起来。B进程即将从内核态返回到用户态时，接收到了信号；
  * 此时B进程仍停留在内核态，B进程根据信号表查找此信号对应的处理函数，操作系统新开堆栈(为了不干扰B进程真正的代码堆栈）并将信号处理函数入栈。B进程返回到用户态运行信号处理函数后，再次返回到内核态，还原为原来的堆栈，然后再次返回用户态，从时钟硬件中断位置等待继续执行。
<br>  
